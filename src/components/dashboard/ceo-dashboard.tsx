          ...
          return (
            ...
            >
              ...
              <div className="mt-5 flex items-end justify-between gap-2">
                ...
                {card.hasRing && (
                  <MiniRing percent={card.value} color={card.ringColor!} delay={delay + 0.2} />
                )}
              </div>
              <p className="mt-2 text-xs text-faint">{card.sub}</p>
            </motion.div>
          );
        })}
      </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projectHealth.slice(0, 6).map((ph, i) => (
              <motion.div key={ph.project.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 + i * 0.05 }}>
                <Link href={`/projects/${ph.project.id}`} className="group block rounded-card border border-border bg-card p-5 transition-colors duration-200 hover:border-border-hover active:scale-[0.99]">
