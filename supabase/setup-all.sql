...
-- 8. Finance Uploads (version history per business)
...
do $$ begin
  ...
  if not exists (select 1 from pg_policies where policyname = 'Finance users can read uploads') then
    create policy "Finance users can read uploads" on public.finance_uploads for select using (exists (select 1 from public.business_units bu where bu.id = finance_uploads.business_id and bu.company_id = public.get_my_company_id()) and public.get_my_role() in ('owner', 'investor'));
  end if;
  if not exists (select 1 from pg_policies where policyname = 'Finance users can insert uploads') then
    create policy "Finance users can insert uploads" on public.finance_uploads for insert with check (uploaded_by = auth.uid() and exists (select 1 from public.business_units bu where bu.id = finance_uploads.business_id and bu.company_id = public.get_my_company_id()) and public.get_my_role() in ('owner', 'investor'));
  end if;
  if not exists (select 1 from pg_policies where policyname = 'Owner can delete uploads') then
    create policy "Owner can delete uploads" on public.finance_uploads for delete using (exists (select 1 from public.business_units bu where bu.id = finance_uploads.business_id and bu.company_id = public.get_my_company_id()) and public.get_my_role() = 'owner');
  end if;
  -- Finance records RLS
  if not exists (select 1 from pg_policies where policyname = 'Finance users can read records') then
    create policy "Finance users can read records" on public.finance_records for select using (exists (select 1 from public.business_units bu where bu.id = finance_records.business_id and bu.company_id = public.get_my_company_id()) and public.get_my_role() in ('owner', 'investor'));
  end if;
  if not exists (select 1 from pg_policies where policyname = 'Finance users can insert records') then
    create policy "Finance users can insert records" on public.finance_records for insert with check (exists (select 1 from public.business_units bu where bu.id = finance_records.business_id and bu.company_id = public.get_my_company_id()) and public.get_my_role() in ('owner', 'investor'));
  end if;
  if not exists (select 1 from pg_policies where policyname = 'Finance users can delete records') then
    create policy "Finance users can delete records" on public.finance_records for delete using (exists (select 1 from public.business_units bu where bu.id = finance_records.business_id and bu.company_id = public.get_my_company_id()) and public.get_my_role() in ('owner', 'investor'));
  end if;
  -- Business units CUD (owners only)
  if not exists (select 1 from pg_policies where policyname = 'Owners can insert BUs') then
    create policy "Owners can insert BUs" on public.business_units for insert with check (company_id = public.get_my_company_id() and public.get_my_role() = 'owner');
  end if;
  if not exists (select 1 from pg_policies where policyname = 'Owners can update BUs') then
    create policy "Owners can update BUs" on public.business_units for update using (company_id = public.get_my_company_id() and public.get_my_role() = 'owner');
  end if;
  if not exists (select 1 from pg_policies where policyname = 'Owners can delete BUs') then
    create policy "Owners can delete BUs" on public.business_units for delete using (company_id = public.get_my_company_id() and public.get_my_role() = 'owner');
  end if;
  -- User sessions RLS
