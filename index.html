...
.checkbox{width:20px;height:20px;border-radius:6px;border:2px solid var(--border);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s;flex-shrink:0}
.checkbox:hover{border-color:var(--accent)}
.checkbox.checked{background:var(--done);border-color:var(--done)}
.checkbox.checked::after{content:'✓';color:white;font-size:.75rem;font-weight:700}
...
@media(max-width:900px){.header{padding:24px 20px 20px}.main{padding:20px 20px 40px}.task-row,.task-header{grid-template-columns:32px 1fr auto;gap:6px}.task-row{padding:12px 16px}.header-left h1{font-size:1.6rem}.login-card{padding:32px 24px}}
</style>
</head>
<body>
...
<div class="login-page" id="loginPage">
  <div class="login-card">
    <div class="login-logo">Task Board</div>
    <div class="login-sub">Sign in to access your projects</div>
...
<div class="header">
  <div class="header-top">
    <div class="header-left"><h1>Task Board</h1><div class="subtitle"><span id="projectCount">0</span> projects · <span id="totalTasks">0</span> tasks</div></div>
...
// ═══ LOGIN ═══
function doLogin(){const u=document.getElementById('loginUser').value.trim(),p=document.getElementById('loginPass').value;const user=USERS.find(x=>x.username.toLowerCase()===u.toLowerCase()&&x.password===p);if(!user){document.getElementById('loginError').textContent='Invalid username or password';return}currentUser=user;document.getElementById('loginPage').style.display='none';document.getElementById('app').classList.add('visible');setupApp()}
function doLogout(){currentUser=null;showBacklog=false;document.getElementById('app').classList.remove('visible');document.getElementById('loginPage').style.display='flex';document.getElementById('loginUser').value='';document.getElementById('loginPass').value='';document.getElementById('loginError').textContent=''}
...
function setupApp(){
  ...
  activeFilter='all';activePersonFilter=null;showBacklog=false;render();
}
...
function renderBacklog(){
  ...
  const byUser={};
  PROJECTS.forEach(p=>{if(!canSeeProject(p))return;p.tasks.filter(t=>t.status==='done').forEach(t=>{const person=t.person||'—';if(!byUser[person])byUser[person]=[];byUser[person].push({...t,projectName:p.name,projectColor:p.color})})});
  if(!Object.keys(byUser).length){container.innerHTML='<div class="backlog-empty">No completed tasks yet.</div>';return}
  Object.entries(byUser).sort((a,b)=>b[1].length-a[1].length).forEach(([person,tasks])=>{
    const sec=document.createElement('div');sec.className='backlog-section';
    sec.innerHTML=`<div class="backlog-user-header"><div class="backlog-avatar">${person.charAt(0)}</div><span class="backlog-name">${esc(person)}</span><span class="backlog-count">${tasks.length} completed</span></div>${tasks.map(t=>`<div class="backlog-task"><span class="bt-check">✓</span><span class="bt-text">${esc(t.text)}</span><span class="bt-project" style="background:${t.projectColor}20;color:${t.projectColor}">${esc(t.projectName)}</span><span class="bt-date">${getDeadlineLabel(t.deadline)}</span></div>`).join('')}`;
...
function render(){
  ...
  PROJECTS.forEach((project,pi)=>{
    ...
    const doneInP=tasks.filter(t=>t.status==='done').length;doneCount+=doneInP;
    progressCount+=tasks.filter(t=>t.status==='progress').length;
    overdueCount+=tasks.filter(t=>isOverdue(t)).length;
    const pct=tasks.length?Math.round((doneInP/tasks.length)*100):0;
